    ...
    <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 h-20 flex items-center justify-between px-8 sticky top-0 z-30 no-print shadow-sm">
      <div className="flex items-center flex-1 max-w-xl">
        <div className="relative w-full group">
          ...
          <input 
            type="text" 
            placeholder="Search Intelligence, Products, or Customers..." 
            className="w-full pl-12 pr-4 py-3 bg-slate-100 border-none rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-medium text-sm shadow-inner"
          />
        </div>
      </div>

      <div className="flex items-center space-x-6">
        <div className="hidden xl:flex items-center space-x-4 border-r border-slate-100 pr-6">
        ...
        <div className="relative">
          <button 
            ...
            className={`p-3 rounded-2xl relative transition-all ${notifications.length > 0 ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}
          >
            <Bell size={20} />
